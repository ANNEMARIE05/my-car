...
import type { StatutVehicule, TypeCarburant, Vehicule } from '../types';
...
export const Vehicules = () => {
  ...
  const [filtreStatut, setFiltreStatut] = useState<StatutVehicule | 'tous'>('tous');
  const [filtreCarburant, setFiltreCarburant] = useState<TypeCarburant | 'tous'>('tous');
  ...
  const handleModifier = (vehicule: Vehicule) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (vehiculeSelectionne) {
      setVehicules(
        vehicules.map((v) => (v.id === vehiculeSelectionne.id ? { ...v, ...formData } as Vehicule : v))
      );
  ...
  const CarteVehicule = ({ vehicule }: { vehicule: Vehicule }) => (
    <motion.div
      layout
    ...
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      {vehiculesFiltres.length > 0 && totalPages > 1 && (
        <Pagination
          ...
          totalItems={vehiculesFiltres.length}
        />
      )}
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Immatriculation</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Kilométrage</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Prix location (FCFA/jour)</label>
                    <input
                      type="number"
                      value={formData.prixLocation}
                      onChange={(e) => setFormData({ ...formData, prixLocation: parseInt(e.target.value) })}
                      className="input-field"
                      required
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, carburant: e.target.value as TypeCarburant })}
                      className="input-field"
                    >
                      <option value="essence">Essence</option>
                      <option value="diesel">Diesel</option>
                ...
                <div className="flex items-center gap-4 pt-4">
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      checked={formData.assuranceValide}
                      onChange={(e) => setFormData({ ...formData, assuranceValide: e.target.checked })}
                      className="w-4 h-4 text-blue-600 rounded"
                    />
                    <span className="text-sm text-slate-700">Assurance valide</span>
                  </label>
                </div>
                <div className="flex gap-4 pt-4">
                  ...
                  >
                    {vehiculeSelectionne ? 'Modifier' : 'Ajouter'}
                  </motion.button>
                </div>
              </form>
            </motion.div>
